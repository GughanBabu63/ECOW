...
#-----------------------DB Insertion Functions :
  ...
  def insert_ticket_setup(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_TICKET_SETUP(GUILD_ID,OPEN_ID,CLOSE_ID,LOG_CHANNEL_ID,TRANSCRIPT_CHANNEL_ID,ROLES_ID) VALUES (?,?,?,?,?,?)'''
    value = (
        value[0],
        value[1],
        value[2],
  ...
  def insert_challonge_role(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_CHALLONGE_ROLE(ROLE_ID)VALUES(?)'''
    value = (value, )
    try:
     self.execute(querry, value)
    except:
  ...
  def insert_log(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_CHALLONGE_LOG(CHANNEL_ID,CHANNEL_NAME,GUILD_ID) VALUES(?,?,?)'''
    value = (
        value[0],
        value[1],
        value[2],
  ...
  def insert_result(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_CHALLONGE_RESULT(CHANNEL_ID,CHANNEL_NAME,GUILD_ID) VALUES(?,?,?)'''
    value = (
        value[0],
        value[1],
        value[2],
  ...
  def insert_testing(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_CHALLONGE_TESTING(CHANNEL_ID,CHANNEL_NAME,GUILD_ID) VALUES(?,?,?)'''
    value = (
        value[0],
        value[1],
        value[2],
  ...
  def insert_tournament(self, value):
    ...
    querry = '''INSERT INTO ECOW_TOURS(Tour_Id,Tour_Name,Tour_Type,
      Tour_Bracket,Participant_Count) VALUES(?,?,?,?,?)'''
    try:
  ...
  def insert_tournament_players(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_PLAYERS(Tour_ID,Player_Id,Player_Main_Id,Player_Name)VALUES(?,?,?,?)'''
    try:
      for i in range(len(value)):
        self.execute(querry, value[i])
    except:
  ...
  def insert_tournament_matches(self, value):
    self.connect()
    querry = '''INSERT INTO ECOW_MATCHES(Match_Name,Tournament_Name,Tour_Id ,Match_Id ,Player1_ID,Player2_Id,Scores,Winner,State)VALUES(?,?,?,?,?,?,?,?,?)'''
    try:
      for i in range(len(value)):
        self.execute(querry, value[i])
...
# fetch all players of a tournament by tournament id
  ...
  def fetch_ticket_setup(self, value):
    self.connect()
    value = (value, )
    querry = '''SELECT * FROM ECOW_TICKET_SETUP WHERE GUILD_ID = ?'''
    self.execute(querry, value)
    k = self.cur.fetchall()
    k = [i for i in k]
    self.commit()
  ...
  def fetch_all_players(self, value):
    self.connect()
    value = (value, )
    querry = '''SELECT * FROM ECOW_PLAYERS WHERE Tour_ID = ?'''
    self.execute(querry, value)
    m = []
  ...
  def fetch_one_tournament(self, value):
    self.connect()
    value = (value, )
    querry = '''SELECT * FROM ECOW_TOURS WHERE Tour_Name = ?'''
    self.execute(querry, value)
    m = []
    for i in self.cur.fetchall():
      m.append(i)
  ...
  def fetch_one_tournament_id(self, value):
    self.connect()
    value = (value, )
    querry = '''SELECT * FROM ECOW_MATCHES WHERE Match_Name = ?'''
    self.execute(querry, value)
    m = []
    for i in self.cur.fetchall():
      m.append(i)
  ...
  def fetch_one_player(self, value):
    ...
    querry = '''SELECT * FROM ECOW_PLAYERS WHERE Player_ID = ? OR Player_Main_Id = ?'''
    self.execute(querry, value)
    m = []
    for i in self.cur.fetchall():
      m.append(i)
  ...
  def fetch_one_player_name(self, value):
    self.connect()
    value = (value, )
    querry = '''SELECT Player_ID,Player_Main_ID FROM ECOW_PLAYERS WHERE Player_Name = ?'''
    self.execute(querry, value)
    m = []
    for i in self.cur.fetchall():
      m.append(i)
